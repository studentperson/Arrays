...
template<class T>
void Array2:: Merge(T left, T lEnd, T right, T rEnd)
{
...
//NM: 031118 code not working; fixing
template<class T>
void Array2::msHelper(T left, T right)
{	
	T middle;
	if (left < right-1)
	{
...
template<class T>
int Array2::Partition(T low, T high)
{
    //int pivot = arrData[high];    // pivot
        T mid, indexLow, indexHigh;
	T pivot;
	int count00;
...
template <class T>
void Array2::quickSort(T low, T high)
{
     if (low < high)
    {
...
template<class T>
T Array2::swapElement(T *inpArr, int index00, int index01)
{	
	T temp;
	temp = inpArr[index00];
...
template<class T>
void Array2::copyElements(T *inpArr, T *retArr, T inpStart, T inpEnd, T retStart) //start is by index end is like length
{
	int count00;
